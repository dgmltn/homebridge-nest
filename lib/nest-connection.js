            ...
            if (propObjects) {
                ...
                propObjects.forEach(propObject => {
                    try {
                        ...
                        console.log('Warning: error enumerating property', propName + '@' + propObject.object.id, error);
                    }
                });
            }
        };
        ...
        try {
            ...
            if (object && object.message && object.message.length > 0) {
                ...
                if (object) {
                        ...
                        if (!peerDevice.object.id.startsWith('STRUCTURE_')) {
                            ...
                            peerDevice.data.property.value.devices.forEach(el => {
                                ...
                                if (body.track && body.track[deviceId] && !body.track[deviceId].online) {
                                    ...
                                    initDevice('detect', deviceId, structureId, el.data.fwVersion, body, deviceType);
                                } else {
                    ...
                    translateProperty(object, 'open_close', null, (openClose, id) => {
                      let deviceKey = this.legacyDeviceMap[id];
                      if (checkDeviceExists(body, id)) {
                          body.detect[id].door_open = openClose.data.property.value.openCloseState == 'OPEN_CLOSE_STATE_OPEN';
                      }
                    });
        ...
        for (const structureId in structures) {
            ...
            swarm.map(unit => unit.split('.')).forEach(unit => {
                try {
                    ...
                    else if (deviceType == 'detect') {
                      let thisDevice = detect[deviceId];
                      thisDevice.device_id = deviceId;
                      thisDevice.where_name = whereLookup[thisDevice.where_id];
                      thisDevice.name = (thisDevice.description || thisDevice.where_name || 'Nest') + ' Detect';
                      data.devices['detects'][deviceId] = detect[deviceId];
                    }
                    else {
                      log.verbose("unknown device type", deviceType);
                    }
                } catch(error) {
